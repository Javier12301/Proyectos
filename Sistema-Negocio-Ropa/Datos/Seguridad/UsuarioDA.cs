...
namespace Datos.Seguridad
{
    public class UsuarioDA
    {
        ...
        public bool VerificarReferencia(int usuarioID)
        {
            bool tieneReferencias = false;
            using (SqlConnection oContexto = conexion.EstablecerConexion())
            {
                SqlCommand cmd = new SqlCommand("VerificarReferenciasUsuario", oContexto);
        ...
        public bool BajaUsuario(int usuarioID)
        {
            ...
            using(SqlConnection oContexto = conexion.EstablecerConexion())
            {
                bool tieneReferencias = VerificarReferencia(usuarioID);

                try
                {
                    if (tieneReferencias)
                    {
                        // tiene referencia, se modificara el estado
                        Usuario usuario = ObtenerUsuarioPorID(usuarioID);
                        usuario.Estado = false;
                        baja = ModificarUsuario(usuario);
                    }
                    else
                    {
                        StringBuilder query = new StringBuilder();
